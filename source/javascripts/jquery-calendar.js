 ...
 *  - Forbids click-navigation up to the month view or navigating
 *    - Commented out weektomonth area of Calendar.prototype.clickSwitchView
 *  - Won't draw arrows to navigate between days in week view
 *    - Commented out relevant areas of Calendar.prototype.weekDrawDays
 *  - Jails navigation to a configured start and end window
 *    - Added start and end to this.conf and updated unixTimestamp to use start when present
 *    - Modified Calendar.prototype.moveCurrentIntervalView
 *  - Removed ctrl key from keyboard navigation shortcuts
 *    - Modified Calendar.prototype.keyboardSwitchView
 *  - Doesn't specially-color modal backdrop and customized modal content
 *    - Commented out rgb-setting code in eventClickDefault
 *    - Altered header generation within the modal body
 *  - Simplifies daynote generation
 *    - Commented out a bunch in Calendar.prototype.drawEventsOrDaynotes about event content
 */
...
jQuery(document).ready(function($){
  ...
  function Calendar(element, Args) {
    ...
    this.conf = {
      ...
      unixTimestamp: (Args.unixTimestamp) ? Args.unixTimestamp : (Args.start) ? Args.start.format('X') : moment().format('X'),
      start: (Args.start) ? Args.start : moment(),
      end: (Args.end) ? Args.end : moment(),
      event: {
        hover: {
      ...
      var eventClickDefault = function(event, self, elem, evt){
        if (!event.isDefaultPrevented()){
          ...
          modal.find('.modal-body').append(
            $('<h4>').append(
              moment($(elem).data('start'), 'X').format('MMMM Do h:mma') + " - " + moment($(elem).data('end'), 'X').format('h:mma')
            )
          );
  ...
  Calendar.prototype.moveCurrentIntervalView = function(direction) {
    ...
    var desiredTarget = moment.unix(self.conf.unixTimestamp);

    if (direction == 'left'){
      desiredTarget = desiredTarget.subtract(1, word);
    }

    if (direction == 'right'){
      desiredTarget = desiredTarget.add(1, word);
    }

    if(desiredTarget < self.conf.start || desiredTarget > self.conf.end) {
      this.conf.unixTimestamp = parseInt(self.conf.start.format('X'));
    } else {
      this.conf.unixTimestamp = parseInt(desiredTarget.format('X'));
    }
